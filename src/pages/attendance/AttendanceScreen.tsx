...
export function AttendanceScreen() {
  ...
  return (
    <div className="pb-24 animate-in fade-in duration-300 flex flex-col items-center pt-8">
      ...
      <h1 className="text-2xl font-bold text-white mb-2 text-center">Gym Attendance</h1>
      ...
      <form onSubmit={handleMarkPresent} className="w-full max-w-sm space-y-4">
        ...
        <Button fullWidth size="lg" type="submit" className={status === 'success' ? 'bg-emerald-500 hover:bg-emerald-600 text-white border-emerald-600' : ''}>
          {status === 'success' ? (
            <>
      ...
      <div className="mt-12 w-full max-w-sm">
        <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-4">Recent Check-ins</h3>
        <div className="space-y-3">
          {['Rahul Sharma', 'Priya Patel', 'Amit Kumar'].map((name, i) => (
            <div key={i} className="flex justify-between items-center bg-zinc-900 border border-zinc-800 p-3 rounded-xl">
              ...
              <span className="text-xs text-emerald-400">10 mins ago</span>
            </div>
          ))}
        </div>
      </div>
