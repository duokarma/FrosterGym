          ...
          {payments.map(payment => (
            <div key={payment.id} className="p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:bg-[#171613]/50 transition-colors">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-sm font-medium text-[#F4F1E8]">{payment.member}</h3>
                  <p className="text-xs text-[#706D66]">{new Date(payment.date).toLocaleDateString()} • {payment.method}</p>
                </div>
              </div>
